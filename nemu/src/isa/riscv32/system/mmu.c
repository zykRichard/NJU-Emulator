...
paddr_t isa_mmu_translate(vaddr_t vaddr, int len, int type) {
  ...
  if(!(PDE & 0x1)){
    panic("PDE invalid when reading addr %x, PDE is %x, vpn0 : %x, vpn1 : %x, vpn_off : %x",vaddr, PDE, vpn0, vpn1, vpn_off);
  ...
  if(!(PTE & 0x1))
    panic("PTE invalid when reading addr %x, PTE is %x, PDE is %x, vpn0: %x, vpn1 : %x, vpn_off : %x", vaddr, PTE, PDE, vpn0, vpn1, vpn_off);
